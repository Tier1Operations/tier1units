...
//diag_log format["TUBE - POS BEFORE PRECISE CHECK: %1", _pos];
...
//diag_log format["TUBE - POS BEFORE GROUND CHECK: %1", _pos];
...
//diag_log format["TUBE - POS AFTER GROUND CHECK: %1", _pos];
...
//diag_log format["TUBE -- typeOf: %1", typeOf _tube];
//diag_log format["TUBE -- _tubes: %1", _tubes];
//diag_log format["TUBE -- _rounds: %1", _rounds];
//diag_log format["TUBE -- _profile: %1", _profile];
//diag_log format["TUBE -- _pos: %1", _pos];
//diag_log format["TUBE -- _warheadType: %1", _warheadType];
//diag_log format["TUBE -- _missionType: %1", _missionType];
//diag_log format["TUBE -- _sheafSize: %1", _sheafSize];
//diag_log format["TUBE -- _fuse: %1", _fuse];
//diag_log format["TUBE -- _assetType: %1", _assetType];
//diag_log format["TUBE -- _sheaf: %1", _sheaf];
//diag_log format["TUBE -- _airburstHeight: %1", _airburstHeight];
//diag_log format["TUBE -- _flightTime: %1", _flightTime];
//diag_log format["TUBE -- _asset: %1", _asset];
//diag_log format["TUBE -- _tubeType: %1", _tubeType];
//diag_log format["TUBE -- _gunAngle: %1", _gunAngle];
//diag_log format["TUBE -- _dtaSelectedTube: %1", _dtaSelectedTube];
//diag_log format["TUBE -- _prePlotted: %1", _prePlotted];
//diag_log format["TUBE -- _sender: %1", _sender];
//diag_log format["TUBE -- _posDisplay: %1", _posDisplay];
//diag_log format["TUBE -- _infoDummyNetID: %1", _infoDummyNetID];
//diag_log format["TUBE -- _tube: %1", _tube];
//diag_log format["TUBE -- _isAnnouncementUnit: %1", _isAnnouncementUnit];
//diag_log format["TUBE -- _GPSZAdjust: %1", _GPSZAdjust];
//diag_log format["TUBE -- _sheafLineDir: %1", _sheafLineDir];
//diag_log format["TUBE -- _sheafLineDist: %1", _sheafLineDist];
...
//diag_log format["TUBE - POS AFTER AIRBURST CHECK: %1", _pos];
...
//diag_log format["TUBE - POS BEFORE MIXED ALTERNATE: %1", _pos];
...
//diag_log format["TUBE - POS AFTER MIXED ALTERNATE: %1", _pos];
...
private _getChargesArray = {
	...
	//diag_log format["TUBE - CHARGES - _warheadType: %1", _warheadType];
	//diag_log format["TUBE - CHARGES - _ammoSpeed: %1", _ammoSpeed];
	
	{
		...
		if (getNumber (configfile >> "CfgWeapons" >> _weapon >> _charge >> "showToPlayer") == 1) then {
			...
			//diag_log format["TUBE - CHARGE: %1 - _calc: %2", _charge, _calc];
			if (_calc < 0) exitWith {
				//diag_log format ["TUBE- IMPOSSIBLE _calc - CHARGE: %1 -- VELOCITY: %2 -- _calc: %3", _charge, _vel, _calc];	
			};
			...
			//diag_log format ["TUBE - CHARGE: %1 -- VELOCITY: %2 -- HIGH ANGLE: %3 -- HIGH ETA: %4 -- LOW ANGLE: %5 -- LOW ETA: %6", _charge, _vel, _hA, _hETA, _lA, _lETA];
		};
...
_findBestCharge = {
	...
	//diag_log format ["TUBE - FIND BEST CHARGE - _chargesArrayHigh: %1", _chargesArrayHigh];
	//diag_log format ["TUBE - FIND BEST CHARGE - _chargesArrayLow: %1", _chargesArrayLow];
	//diag_log format ["TUBE - FIND BEST CHARGE - _chargesArray: %1", _chargesArray];
	...
	_findCharge = {
		...
		{
			//diag_log "TUBE - FIND CHARGE - FOREACH START";
			...
			//diag_log format["TUBE - FIND CHARGE - CHARGE: %1", _x];
			//diag_log format["TUBE - FIND CHARGE - _angleA: %1", _angleA];
			//diag_log format["TUBE - FIND CHARGE - _posA: %1", _posA];
			//diag_log format["TUBE - FIND CHARGE - _posC: %1", _posC];
			//diag_log format["TUBE - FIND CHARGE - _distanceAdj: %1", _distanceAdj];
			//diag_log format["TUBE - FIND CHARGE - _angleB: %1", _angleB];
			//diag_log format["TUBE - FIND CHARGE - _distanceOp: %1", _distanceOp];
			//diag_log format["TUBE - FIND CHARGE - _heightArtyToSeaLvl: %1", _heightArtyToSeaLvl];
			//diag_log format["TUBE - FIND CHARGE - _x1: %1", _x1];
			//diag_log format["TUBE - FIND CHARGE - _y: %1", _y];
			//diag_log format["TUBE - FIND CHARGE - _z: %1", _z];
			...
			//diag_log format ["TUBE - FIND BEST CHARGE - FIND CHARGE - CHARGE: %1 - _angleA: %2 - _verDegrees: %3 - abs(_angleA-_verDegrees): %4", _x select 0, _angleA, _verDegrees, abs(_angleA - _verDegrees)];
			//diag_log format ["TUBE - FIND BEST CHARGE - FIND CHARGE - WORLD: %1 - _maxDifference: %2", worldName, _maxDifference];
			...
			if (_difference < _maxDifference and _difference > -_maxDifference) then {
				...
				//diag_log format ["TUBE - FIND BEST CHARGE - FIND CHARGE - _chosenCharge: %1", _chosenCharge];
				...
				//diag_log format ["TUBE - FIND BEST CHARGE - _pos: %1", _pos];
				//diag_log format ["TUBE - FIND BEST CHARGE - _lineIntersectsSurfaces: %1", _lineIntersectsSurfaces];
				
				// Obstructed.
				if (count _lineIntersectsSurfaces > 0) then {
					_obstructed = true;
					_chargeFound = false;
					//diag_log format ["TUBE - FIND BEST CHARGE - OBSTRUCTED - _chosenCharge: %1", _chosenCharge];
				};
			...
			if (_chargeFound) exitWith {
				//diag_log "TUBE - FIND CHARGE - CHARGE FOUND";
			};
			
			//diag_log "TUBE - FIND CHARGE - FOREACH END";
	...
	if (!_chargeFound and !_longRangeGuided and !_abort) then {
		...
		//diag_log format ["TUBE - FIND BEST CHARGE - TRYING OTHER ANGLE - _chargesArray: %1", _chargesArray];
	};
	
	
	if (_abort) then {
		_chosenCharge = ["",0,0,"",false,true];
		//diag_log format ["TUBE - FIND BEST CHARGE - ABORT - _chosenCharge: %1", _chosenCharge];
	} else {
		if (_chosenCharge select 0 == "") then {
			_chosenCharge = ["",0,0,"",false,false];
			//diag_log format ["TUBE - FIND BEST CHARGE - NOTHING CHOSEN - _chosenCharge: %1", _chosenCharge];
		} else {
			_chosenCharge pushback _abort;
			//diag_log format ["TUBE - FIND BEST CHARGE - END - _chosenCharge: %1", _chosenCharge];
		};
	};
...
while {_rounds > 0} do {
	...
	if (!_outOfAmmo and !_impossibleShot and !_abort) then {
		...
		//diag_log format["TUBE - FIRING LOOP - _fireCenterFirst: %1", _fireCenterFirst];
		//diag_log format["TUBE - FIRING LOOP - _scatter: %1", _scatter];
		//diag_log format["TUBE - FIRING LOOP - _scatter2: %1", _scatter2];
		//diag_log format["TUBE - FIRING LOOP - POS BEFORE SHEAF: %1", _pos];
		...
		if (_scatter > 0) then {
			...
			if (!_fireCenterFirst) then {
				...
				sleep 0.1;
				//diag_log format["TUBE - FIRING LOOP - SHEAF NORMAL - _chosenTargetPos: %1 - _sheaf: %2", _chosenTargetPos, _sheaf];
				...
				//diag_log format["TUBE - FIRING LOOP - SHEAF FIRECENTERFIRST - _chosenTargetPos: %1", _chosenTargetPos];
			};
			
		} else {
			
			_chosenTargetPos = _pos;
			//diag_log format["TUBE - FIRING LOOP - NO SHEAF - _chosenTargetPos: %1", _chosenTargetPos];
		};
		
		//diag_log format["TUBE - FIRING LOOP - CHOSENTARGETPOS AFTER SHEAF: %1", _chosenTargetPos];
		...
		if (!DTA_turnOffRandomSpread and {_action != 1} and {_action != 2} and {_action != 3}) then {
			...
			//diag_log format["TUBE - FIRING LOOP - SPREAD AFTER IFS: %1", _scatterSpread];
			...
			//diag_log format["TUBE - FIRING LOOP - SPREAD AFTER CALC: %1", _scatterSpread];
		...
		//diag_log format["TUBE - FIRING LOOP - CHOSENTARGETPOS AFTER RANDOM SPREAD: %1", _chosenTargetPos];
		...
		//diag_log format["TUBE - FIRING LOOP -- _longRangeGuided: %1", _longRangeGuided];
		//diag_log format["TUBE - FIRING LOOP -- _distance: %1", _distance];
		//diag_log format["TUBE - FIRING LOOP -- VECTORDISTANCE: %1", _chosenTargetPos vectorDistance (getPosASL _tube)];
		
		//diag_log "TUBE - CHARGES START";
		//diag_log format["TUBE - CHARGES -- MODES: %1", (getArray (configfile >> "CfgWeapons" >> _weapon >> "modes"))];
		...
		if (!_longRangeGuided) then {
			...
			//diag_log format["TUBE - CHARGE NORMAL -- _chargesArrayLow: %1", _chargesArrayLow];
			//diag_log format["TUBE - CHARGE NORMAL -- _chargesArrayHigh: %1", _chargesArrayHigh];
			...
			//diag_log format["TUBE - CHARGE LONG RANGE GUIDED -- _chargesArrayLow: %1", _chargesArrayLow];
			//diag_log format["TUBE - CHARGE LONG RANGE GUIDED -- _chargesArrayHigh: %1", _chargesArrayHigh];
		};
		...
		if (count _chargesArrayLow == 0 and {count _chargesArrayHigh == 0}) then {
			...
			//diag_log format["TUBE - ABORT - IMPOSSIBLE SHOT 1 - Check1: %1 - Check2: %2", count _chargesArrayLow, count _chargesArrayHigh];
		};
	};
	...
	if (_firstRound) then {
		...
		//diag_log format["TUBE - FIRING LOOP -- CLOSETOTRP: %1", _closeToTRP];
		...
		if (((_lastFiringPos vectorDistance _chosenTargetPos < 300) and (_lastGunAngle == _gunAngle)) or _prePlotted or _closeToTRP) then {
			...
			//diag_log format["TUBE - FIRING LOOP -- HAS FIRED PREVIOUSLY ON POS - CHECK1: %1 - CHECK2: %2 - CHECK3: %3 - CHECK4: %4 - CHECK5: %5", _lastFiringPos vectorDistance _chosenTargetPos, _lastGunAngle, _gunAngle, _prePlotted, _closeToTRP];
			...
			//diag_log format["TUBE - FIRING LOOP -- HAS NOT FIRED PREVIOUSLY ON POS - CHECK1: %1 - CHECK2: %2 - CHECK3: %3 - CHECK4: %4 - CHECK5: %5", _lastFiringPos vectorDistance _chosenTargetPos, _lastGunAngle, _gunAngle, _prePlotted, _closeToTRP];
	...
	if (_bestCharge select 0 == "" or _obstructed) then {
		...
		if (!_abort) then {
			...
			if (!_obstructed) then {
				...
				//diag_log format["TUBE - ABORT - IMPOSSIBLE SHOT 2 - Check1: %1", !_obstructed];
			} else {
				[_tube,"Unable to get a firing solution - shot obstructed.","beep"] call dta_fnc_SendComms;
				//diag_log format["TUBE - ABORT - IMPOSSIBLE SHOT 3 - Check1: %1", !_obstructed];
			};
		};
		...
		if (_bestCharge select 3 != _gunAngle) then {
			...
			//diag_log format["TUBE - ABORT - IMPOSSIBLE SHOT 4 - Check1: %1 - Check2: %2", _bestCharge select 3, _gunAngle];
		};
	};
	...
	if (_firstRound and {!_outOfAmmo and !_impossibleShot and !_abort}) then {
		...
		//diag_log format["TUBE - CHOSEN CHARGE -- _angle: %1", _angle];
		//diag_log format["TUBE - CHOSEN CHARGE -- _ETA: %1", _ETA];
		
		// When using the guided script, the ETA times need to change a bit.
		if (_longRangeGuided) then {
			_ETA = (_ETA/2) + ((_distance-(_regularMaxRange/2))/(_vel*0.4));
			//diag_log format["TUBE - CHANGING ETA LONG RANGE GUIDED -- _ETA: %1", _ETA];
		} else {
			if (_action == 1 or {_action == 2} or {_action == 3}) then {
				_ETA = _ETA * 1.05;
				//diag_log format["TUBE - CHANGING ETA FOR ACTION 1,2,3 -- _ETA: %1", _ETA];
			};
		};
		...
		//diag_log format["TUBE - DTA_ETA: %1", _tube getVariable ["DTA_ETA", 9999]];
	};
	...
	//diag_log format["TUBE - FIRING LOOP - POS BEFORE AIRBURST IF: %1", _pos];
	...
	if (!_outOfAmmo and !_impossibleShot and !_abort) then {
		...
		//diag_log format["TUBE - FIRING LOOP - POS AFTER AIRBURST IF: %1", _pos];
		...
		if ((currentMagazine _tube) == _warheadType) then {
			...
			if (_action != 1 and {_action != 2 and {_action != 3}}) then {
				...
				if (_isAnnouncementUnit) then {
					private _EH = _tube addEventHandler ["fired", {
						...
						//diag_log format["TUBE - UPDATE TARGET POS - EH FIRED - TUBE: %1", _tube];
						...
						[_tube, _this select 6] spawn {
							...
							if (isNil "_spotter") exitWith {
								//diag_log format["TUBE - UPDATE TARGET POS - ABORT1 - TUBE: %1 - CHECK1: %2", _tube, _spotter];
							};
							...
							if (isNil "_spotter" or {isNil "_spotter2" or {_spotter != _spotter2}}) exitWith {
								//diag_log format["TUBE - UPDATE TARGET POS - ABORT2 - TUBE: %1 - CHECK1: %2 - CHECK2: %3", _tube, _spotter, _spotter2];
							};
							...
							//diag_log format["TUBE - UPDATE TARGET POS - END - TUBE: %1 - POS: %2 - SPOTTER: %3", _tube, _pos, _spotter2];
						};
			...
			//diag_log format["TUBE - ABORT - WRONG MAG LOADED - CURMAG: %1 - _warheadType: %2", currentMagazine _tube, _warheadType];
			
			if (!_outOfAmmo) then {
				//diag_log format["TUBE - ABORT - OUT OF AMMO - Check1: %1", !_outOfAmmo];
				_outOfAmmo = true;
	...
	//diag_log format["TUBE - FIRING LOOP END -- _firstRound: %1", _firstRound];
	//diag_log format["TUBE - FIRING LOOP END -- _isAnnouncementUnit: %1", _isAnnouncementUnit];
...
//diag_log format["TUBE - ENDING -- _endMission: %1", _endMission];
//diag_log format["TUBE - ENDING -- _abort: %1", _abort];
//diag_log format["TUBE - ENDING -- _outOfAmmo: %1", _outOfAmmo];
//diag_log format["TUBE - ENDING -- _impossibleShot: %1", _impossibleShot];
//diag_log format["TUBE - ENDING -- _allOutOfAmmo: %1", _allOutOfAmmo];
//diag_log format["TUBE - ENDING -- _allImpossibleShot: %1", _allImpossibleShot];
//diag_log format["TUBE - ENDING -- _allAborted: %1", _allAborted];
//diag_log format["TUBE - ENDING -- _isAnnouncementUnit: %1", _isAnnouncementUnit];
//diag_log format["TUBE - ENDING -- _asset: %1", _asset];
...
if (_abort and !_isAnnouncementUnit) exitWith {
	...
	//diag_log format["TUBE END - TUBES INOPERABLE: %1", _amountAborted];
};
...
if (_isAnnouncementUnit) then {
	...
	//diag_log format["TUBE - FINAL ANNOUNCEMENT - dtaAssetsBusy: %1", dtaAssetsBusy];
	//diag_log format["TUBE - FINAL ANNOUNCEMENT - _asset: %1", _asset];
		...
		if (!_allImpossibleShot) then {
			if (_missionType == "FFE") then {
				if (!_allOutOfAmmo) then {
					...
					//diag_log format["TUBE END - NORMAL ENDING: %1", !_allOutOfAmmo];
				} else {
					...
					//diag_log format["TUBE END - RAN OUT OF AMMO ENDING: %1", !_allOutOfAmmo];
				};
			};
			...
			//diag_log format["TUBE END - IMPOSSIBLE SHOT ENDING: %1", !_allImpossibleShot];
		};
	};
};
