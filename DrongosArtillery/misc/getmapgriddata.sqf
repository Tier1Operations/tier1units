...
if (_letterGrid) exitWith {
	//hint format["Map Grid Warning (%1)\n\nMap uses letter grids\n\n[%2, %3]",worldName,_formatX,_formatY];
};
...
if (_stepYat5 < 0) then {
	//hint format["Map Grid Warning (%1)\n\nNorthing is reversed.",worldName];
};

if (_stepXat5 != 1) then {
	//hint format["Map Grid Warning (%1)\n\nMGRS 10 digit grid does not equal 1 meter:\n(%2) for x.",worldName,_stepXat5];
};

if (_stepYat5 != 1 && {_stepYat5 != -1}) then {
	//hint format["Map Grid Warning (%1)\n\nMGRS 10 digit grid does not equal 1 meter:\n(%2) for y.",worldName,_stepXat5];
};

dta_mapGridData = [_offsetX, _realOffsetY, _stepXat5, _stepYat5];