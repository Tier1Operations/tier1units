...
switch (_adjustMode) do {
	case (1) : {
		...
		if (T1AM_Debug_Mode) then {[_targetPos2D_1, 120, "ColorOrange"] spawn T1AM_Fnc_PlaceMarker};
		
		// Position shown in menu, which could be inaccurate (on purpose).
		private _targetPos2D_2 = [T1AM_Xdisplay,T1AM_Ydisplay,0];
		if (T1AM_Debug_Mode) then {[_targetPos2D_2, 120, "ColorPink"] spawn T1AM_Fnc_PlaceMarker};
		...
		if (T1AM_Debug_Mode) then {[_targetPos2D_1, 120, "ColorBrown"] spawn T1AM_Fnc_PlaceMarker};
		if (T1AM_Debug_Mode) then {[_targetPos2D_2, 120, "ColorRed"] spawn T1AM_Fnc_PlaceMarker};		
	...
	case (2) : {
		...
		// Actual impact location.
		private _targetPos2D_1 = [T1AM_Xdisplay,T1AM_Ydisplay,0];
		if (T1AM_Debug_Mode) then {[_targetPos2D_1, 120, "ColorOrange"] spawn T1AM_Fnc_PlaceMarker};
		
		T1AM_Xdisplay = T1AM_Xdisplay + _DifX;
		T1AM_Ydisplay = T1AM_Ydisplay + _DifY;
		
		_targetPos2D_1 = [T1AM_Xdisplay,T1AM_Ydisplay,0];
		if (T1AM_Debug_Mode) then {[_targetPos2D_1, 120, "ColorBrown"] spawn T1AM_Fnc_PlaceMarker};
		
		
		// Position shown in menu, which could be inaccurate (on purpose).
		private _targetPos2D_2 = [T1AM_X,T1AM_Y,0];
		if (T1AM_Debug_Mode) then {[_targetPos2D_2, 120, "ColorPink"] spawn T1AM_Fnc_PlaceMarker};
		
		T1AM_X = T1AM_X + _DifX;
		T1AM_Y = T1AM_Y + _DifY;
		
		_targetPos2D_2 = [T1AM_X,T1AM_Y,0];
		if (T1AM_Debug_Mode) then {[_targetPos2D_2, 120, "ColorRed"] spawn T1AM_Fnc_PlaceMarker};
	};
};