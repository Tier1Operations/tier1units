...
DEBUGLOG format ["EDIT AIMPOINT POS EVENT START | _this: %1", _this];

private _nr = ctrlText _displayorcontrol;
if (_nr == "") exitWith {};

DEBUGLOG format ["EDIT AIMPOINT POS EVENT 1 | _nr: %1", _nr];

_nr = [_nr] call T1AM_Fnc_GridToPos;
_nr = [_nr, 5, true] call T1AM_Fnc_ParseNumber;

DEBUGLOG "EDIT AIMPOINT POS EVENT 2";
...
DEBUGLOG format ["EDIT AIMPOINT POS EVENT 3 | _nr: %1", _nr];
...
DEBUGLOG format ["EDIT AIMPOINT POS EVENT END | T1AM_LastAimpointX: %1 | T1AM_LastAimpointY: %2", T1AM_LastAimpointX, T1AM_LastAimpointY];