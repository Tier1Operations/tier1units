...
DIAG_LOG format ["EDIT TRP POS EVENT START | _this: %1", _this];

private _nr = ctrlText _displayorcontrol;
if (_nr == "") exitWith {};

DIAG_LOG format ["EDIT TRP POS EVENT 1 | _nr: %1", _nr];

_nr = [_nr] call T1AM_Fnc_GridToPos;
_nr = [_nr, 5, true] call T1AM_Fnc_ParseNumber;

DIAG_LOG "EDIT TRP POS EVENT 2";
...
DIAG_LOG format ["EDIT TRP POS EVENT 3 | _nr: %1", _nr];
...
DIAG_LOG format ["EDIT TRP POS EVENT END | T1AM_LastTRPX: %1 | T1AM_LastTRPY: %2", T1AM_LastTRPX, T1AM_LastTRPY];