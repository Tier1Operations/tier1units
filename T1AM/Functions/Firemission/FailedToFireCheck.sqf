...
DIAG_LOG format["FAILED TO FIRE | AFTER LOOP | _tube: %1 | _asset: %2 | _abort: %3", _tube, _asset, _abort];

if (_abort) exitWith {};

if (!(_tube getVariable ["T1AM_hasFired", false])) then {
	_tube setVariable ["T1AM_failedToFire", true];		// This will abort the mission.
	DIAG_LOG format["FAILED TO FIRE | FAILED | _tube: %1 | _asset: %2 | _abort: %3", _tube, _asset, _abort];
};