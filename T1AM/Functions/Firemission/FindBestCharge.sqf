...
DEBUGLOG format ["TUBE: %1 - FIND BEST CHARGE - _this: %2", _tube, _this];
...
DEBUGLOG format ["TUBE: %1 - FIND BEST CHARGE - _chargesArrayHigh: %2", _tube, _chargesArrayHigh];
DEBUGLOG format ["TUBE: %1 - FIND BEST CHARGE - _chargesArrayLow: %2", _tube, _chargesArrayLow];
DEBUGLOG format ["TUBE: %1 - FIND BEST CHARGE - _chargesArray: %2", _tube, _chargesArray];
...
if (!_chargeFound and !_longRangeGuided and !_abort and !_endMission and !_checkFire) then {
	...
	DEBUGLOG format ["TUBE: %1 - FIND BEST CHARGE - TRYING OTHER ANGLE - _chargesArray: %2", _tube, _chargesArray];
};
...
switch true do {
	case (_abort or _endMission or _checkFire) : {
		...
		DEBUGLOG format ["TUBE: %1 | FIND BEST CHARGE | ABORT | _chosenCharge: %2", _tube, _chosenCharge];
	};
	case (_chosenCharge select 0 == "") : {
		_chosenCharge = ["",0,0,"",false,false];
		DEBUGLOG format ["TUBE: %1 | FIND BEST CHARGE | NOTHING CHOSEN | _chosenCharge: %2", _tube, _chosenCharge];
	};
};

DEBUGLOG format ["TUBE: %1 | FIND BEST CHARGE | END | _chosenCharge: %2 | _abort: %3 | _endMission: %4 | _checkFire: %5", _tube, _chosenCharge, _abort, _endMission, _checkFire];
