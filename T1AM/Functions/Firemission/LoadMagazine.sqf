...
if (!_alreadyLoaded) then {
	...
	if (_waitingTime > 45) then {
		private _spotter = (group _tube) getVariable ["T1AM_controlledBy", objNull];
		[_tube, _spotter, format["Preparing the required ammo will take a while: %1 seconds. Out.",_reloadTime], 7] call T1AM_Fnc_SendComms;
	};
};
