...
private _check = {
	
	{
		...
		private _T1AM_ApproveVehiclesOverride = if (_typeOf in T1AM_ApproveVehiclesOverride) then {true} else {false};
		if (((getNumber (configFile >> "CfgVehicles" >> _typeOf >> "artilleryScanner")) == 1) or _T1AM_ApproveVehiclesOverride) then {
			if (!unitIsUAV _x or _T1AM_ApproveVehiclesOverride) then {
				_x setVariable ["T1AM_suitableVehicle", true, true];
				if (unitIsUAV _x and _x getVariable ["T1AM_disableUAV", true]) then {
					_x setVariable ["T1AM_disableUAV", false];
