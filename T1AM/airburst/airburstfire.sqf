...
switch (_tubeType) do {
	case "82mmMortar": {
		...
		_maxRange = 200;
		_falloff = 2;
		_airburstClass = "T1AM_Airburst_Mortar";
	};
	...
	case "155mmHowitzer": {
		...
		_maxRange = 250;
		_falloff = 2;
		_airburstClass = "T1AM_Airburst_Cannon";
	};
	...
	case "230mmRockets": {
		...
		_maxRange = 300;
		_falloff = 2;
		_airburstClass = "T1AM_Airburst_Rocket";
	};
...
if (_airburstHeight == "MED" or {_airburstHeight == "LOW"} or {_airburstHeight == "HIGH"}) then {
	...
	if (!isNil "ace_frag_fnc_fired") then {
		...
		[_pos,_explosionSize,_shrapnelConcentration,_effectiveRange,_maxRange,_fallOff] execVM "T1AM\Airburst\AirburstHit.sqf";
