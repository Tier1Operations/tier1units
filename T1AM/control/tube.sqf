...
while {_rounds > 0} do {
	...
	if (!_outOfAmmo and !_impossibleShot and !_abort) then {
		...
		if (_scatter > 0) then {
			...
			if (!_fireCenterFirst) then {
				switch true do {
					case (_sheaf == "CIRCLE") : {_chosenTargetPos = [_tube,_asset,_pos,_scatter] call T1AM_Fnc_CircularSheaf};
					case (_sheaf == "BOX") : {_chosenTargetPos = [_tube,_asset,_pos,_scatter,_scatter2] call T1AM_Fnc_BoxSheaf};
					case (_sheaf == "LINE") : {_chosenTargetPos = [_tube,_asset,_pos,_sheafLineDir,_sheafLineDist] call T1AM_Fnc_LineSheaf};
	...
	if (!_outOfAmmo and !_impossibleShot and !_abort) then {
		...
		if (_fuse == "AIRBURST") then {
			...
			_airburstEH = _tube addEventHandler ["fired",compile format ["[_this,%1,'%2',%3,'%4','%5',%6] execVM 'T1AM\Airburst\AirburstFire.sqf'",_chosenTargetPos,_gunAngle,_action,_infoDummyNetID,_airburstHeight,_GPSZAdjust]];
